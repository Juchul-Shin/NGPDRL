...
:precondition (and (at ?p) (at_soil_sample ?p) (empty) (mode) (ssend)
                )
...
:effect (and (not (empty)) (full) (have_rock_analysis ?p) (not (at_rock_sample ?p)) (not (mode))
                )
)
...
(:action communicate_soil_data
 :parameters (?p - waypoint ?x - waypoint) 
 :precondition (and (at ?x)(at_lander ?x)(have_soil_analysis ?p) (mode)
            )
 :effect (and (communicated_soil_data ?p) (not (have_soil_analysis ?p)) (not (ssend))
          )
)
...
(:action take_image
 :parameters ( ?p - waypoint )
 :precondition (and (calibrated ?p) (sended)
