...
:effect (and (not (empty)) (full) (have_soil_analysis ?p) (not (at_soil_sample ?p)) (not (mode))
                )
)

(:action sample_rock
:parameters (?p - waypoint)
:precondition (and (at ?p) (at_rock_sample ?p) (empty) (ssend) (mode)
                )
...
(:action communicate_rock_data
 ...
 :precondition (and (at ?x)(at_lander ?x)(have_rock_analysis ?p) (mode)
            )
 :effect (and (communicated_rock_data ?p) (not (have_rock_analysis ?p)) (not (ssend))
          )
)
